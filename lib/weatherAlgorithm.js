...
function heuristicVals(weightedData){
  ...
  for (key in weightedData) {
    if (key != "an") {
      ...
      console.log(key, weightedData[key].value);
    };
  };
...
function calculateSunset(user, cloud, air, swx) {
  ...
  let dataObj = { chance: chance, clouds: cloudCoverage, air: airParticles, sunsetwx: swx, position: {lat: user.latitude, long: user.longitude}}
